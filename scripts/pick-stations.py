# credit to https://github.com/shrirambharath/weather-data
...
def get_selected_stations_filename(data_dir, append_date):	
	_selected_stations_file = '%s/ghcnd-selected-stations.csv' % (data_dir)
...
def pick_stations(data_dir, start_year):
	...
	#			 }
	result_map = _selected_station_details
	...
	csv_columns = ['lat','lon','station','start','latest','name']
	try:
		with open(_selected_stations_filename, 'w') as csvfile:
